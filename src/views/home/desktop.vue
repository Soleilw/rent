<template>
  <div v-loading="loading" element-loading-text="拼命加载中">
    ...
    <!-- 分页 社区-->
    <div class="block" v-if="area != '' && lets == ''">
      <el-pagination @current-change="areaCurrentChange" :current-page.sync="areaCurrent"
        :page-sizes="[10, 20, 30, 40, 50]" :page-size="areaSize" layout="sizes, prev, pager, next, jumper"
        :total="areaTotal" @size-change="areaSizeChange"></el-pagination>
    </div>
    <!-- 分页 房屋-->
    <div class="block" v-if="area != '' && lets != ''">
      <el-pagination @current-change="letsCurrentchange" :current-page.sync="letsCurrent"
        :page-sizes="[10, 20, 30, 40, 50]" :page-size="letsSize" layout="sizes, prev, pager, next, jumper"
        :total="letsTotal" @size-change="letsSizeChange"></el-pagination>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取统计列表
      fucSstatistics(cur, list) {
        var self = this;
        API.statistics(cur, list)
          .then((res) => {
            self.loading = false;
      ...
      getSstatistics() {
        ...
        self.fucSstatistics(self.current, self.size);
      },
      // 全部 分页
      currentChange(val) {
        var self = this;
        self.current = val;
        self.loading = true;
        self.fucSstatistics(val, self.size);
      },
      // 每页几条
      sizeChange(val) {
        var self = this;
      ...
      // 选择社区
      fucArea(cur, list, area_id) {
        var self = this;
        API.statistics(cur, list, area_id)
          .then((res) => {
            self.loading = false;
            self.tableData = res.data;
      ...
      changeAreaType(value) {
        ...
        self.areaCurrent = 1;
        self.loading = true;
        self.fucArea(self.areaCurrent, self.areaSize, self.area_id);
        self.getLet();
      },
      // 选择社区后 分页
      areaCurrentChange(val) {
        ...
        self.loading = true;
        self.fucArea(val, self.areaSize, self.area_id);
      },
      areaSizeChange(val) {
        var self = this;
        self.areaSize = val;
        self.loading = true;
        self.fucArea(1, val, self.area_id);
        self.areaCurrent = 1;
      },

      // 选择出租屋
      fucLet(cur, list, area_id, address_id) {
        var self = this;
        API.statistics(cur, list, area_id, address_id)
          .then((res) => {
            self.loading = false;
            self.tableData = res.data;
      ...
      letType(value) {
        ...
        self.loading = true;
        self.letsCurrent = 1;
        self.fucLet(self.letsCurrent, self.letsSize, self.area_id, self.address_id)
      },
      // 选择出租屋后 分页
      letsCurrentchange(val) {
        ...
        self.loading = true;
        self.fucLet(val, self.letsSize, self.area_id, self.address_id);
      },
      letsSizeChange(val) {
        var self = this;
        self.letsSize = val;
        self.loading = true;
        self.fucLet(1, val, self.area_id, self.address_id);
        self.letsCurrent = 1;
      },
        ...
        API.statistics(1, 10000).then((res) => {
          ...
          barChart.on("click", function (params) {
            ...
            barChart.dispatchAction({
              type: "dataZoom",
              startValue: self.nameList[Math.max(params.dataIndex - zoomSize / 2, 0)],
              endValue: self.nameList[
                Math.min(
