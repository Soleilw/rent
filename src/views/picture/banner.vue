<template>
  <div v-loading="loading" element-loading-text="拼命加载中">
    ...
    <el-dialog title="上传图片" :visible.sync="dialogBanner" :close-on-click-modal="false" width="500px">
      <div class="box">
        <el-form :model="form">
          <el-form-item>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getBanner(this.current, this.size);
    },
    methods: {
      // 获取轮播图
      getBanner(cur, list) {
        var self = this;
        API.banners(cur, list)
          .then((res) => {
            self.loading = false;
      ...
      toDel() {
        ...
        API.delBanner(self.id).then((res) => {
          ...
          self.dialogDel = false;
          self.getBanner(self.current, self.size);
        });
      },
      // 上传图片
      upload() {
