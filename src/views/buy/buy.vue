            ...
            <div class="service">
              <div v-for="(item,index) in serviceList" :key="index">
                <el-checkbox-group v-model="form.service" class="service-item">
                  <el-checkbox :label="item" :true-labe="item.title" @change="oneChange(item.title)">{{item.name}}
                  </el-checkbox>
                </el-checkbox-group>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.getBuys(this.current, this.size);
    },
    methods: {
      // 获取服务列表
      getBuys(cur, list) {
      ...
      newBuy() {
        ...
        API.buy(self.form).then((res) => {
          ...
          self.getBuys(self.current, self.size);
          self.form = {};
          self.form.service = [];
        });
      },
      ...
      // 搜索
      fucSearch(cur, list, keyword) {
        var self = this;
        API.server(cur, list, self.product_id, keyword).then(
          (res) => {
      ...
      search() {
        var self = this;
        self.orderCurrent = 1;
        self.fucSearch(self.orderCurrent, self.orderSize, self.keyword);
      },
      getServer(cur, list) {
        var self = this;
        API.server(cur, list, self.product_id).then((res) => {
      ...
      orderCurrentChange(val) {
        ...
        if (self.keyword == "") {
          ...
          self.fucSearch(val, self.orderSize, self.keyword);
        }
      },
      // 当前分页
      orderSizeChange(val) {
        ...
        if (self.keyword == "") {
          ...
          self.fucSearch(1, val, self.keyword);
        }
        self.orderCurrent = 1;
      },
