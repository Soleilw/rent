<template>
  <div v-loading="loading" element-loading-text="拼命加载中">
    ...
    <div class="handle-box">
      <div class="btn">
        <el-input v-model="renter_name" placeholder="输入用户名/地址" class="input-with-select"
          ...
          <el-select v-model="type" placeholder="请选择搜索方式" @change="changeType" slot="prepend" style="width: 150px">
        ...
        <template slot-scope="scope">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <el-button size="mini" type="primary" @click="handleRec(scope.$index, scope.row)">返现记录</el-button>
              </el-dropdown-item>
              <el-dropdown-item>
                <el-button size="mini" type="primary" @click="handleRefund(scope.$index, scope.row)">手动返现</el-button>
              </el-dropdown-item>
              <el-dropdown-item>
    ...
    <!-- 添加白名单 -->
    <el-dialog title="添加白名单" :visible.sync="dialogList" :close-on-click-modal="false" width="800px" @close="closeList">
      <div class="box">
          ...
          <div v-for="(item, index) in listForm.service" :key="index">
            <el-form-item v-if="isAdd" label="选择省份">
              ...
              <el-select v-model="pro_id" placeholder="请选择省份" @change="proChange">
                <el-option v-for="item in proList" :key="item.index" :label="item.title" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item v-if="isAdd" label="选择市级">
              <!-- <span>选择市级: </span> -->
              <el-select v-model="city_id" placeholder="请选择市级" @change="cityChange">
                <el-option v-for="item in cityList" :key="item.index" :label="item.title" :value="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item v-if="isAdd" label="选择区级">
              <!-- <span>选择区级: </span> -->
              <el-select v-model="community_id" placeholder="请选择区级" @change="areasChange">
                <el-option v-for="item in communityList" :key="item.index" :label="item.title" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item v-if="isAdd" label="选择社区">
              <!-- <span>选择社区：</span> -->
              <el-select v-model="item.area_id" placeholder="请选择社区" @change="changeAreaType">
            ...
            <el-form-item label="选择身份">
              <el-select v-model="item.type" @change="selectType" :disabled="isDisabled" placeholder="请选择身份类型">
                <el-option v-for="item in tyList" :label="item.name" :value="item.type" :key="item.index"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="选择用户">
              <el-select v-model="item.user_id" filterable placeholder="请选择用户" :disabled="isDisabled">
                <el-option v-for="item in userList" :label="item.snapshot.name" :value="item.user_id" :key="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="返现状态">
              <el-select v-model="item.state" :disabled="isDisabled" placeholder="请选择状态">
                <el-option v-for="item in stateList" :label="item.name" :value="item.value" :key="item.index">
                </el-option>
              </el-select>
            </el-form-item>
          </div>
    ...
    <el-dialog title="返现记录" :visible.sync="dialogRec" width="80%">
      ...
      <!-- 分页 -->
      <div class="block">
        <el-pagination @current-change="recordCurrentChange" :current-page.sync="recordCurrent" :page-size="recordSize"
          layout="sizes, prev, pager, next, jumper" :total="recordTotal" @size-change="recordSizeChange">
        </el-pagination>
      </div>
    </el-dialog>
